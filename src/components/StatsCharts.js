  ...
  const BarChartSection = ({ title, unit, dataKey, limit, isProtein = false, isFasting = false, maxScaleMultiplier = 1.5 }) => {
    ...
    return (
      <section className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
        ...
        <div className="relative w-full overflow-x-auto hide-scrollbar pb-2 pt-2">
          <div className={`min-w-full flex flex-col ${range === 'monthly' ? 'w-[800px]' : ''}`}>
            
            {/* Bars & Target Line Container */}
            <div className={`relative h-[130px] w-full flex items-end justify-between ${range === 'monthly' ? 'gap-2' : 'gap-4'} mt-4`}>
              ...
              {data.map((day, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center relative z-10 flex-1 min-w-[20px] h-full justify-end">
                    <div className="text-[8px] font-bold text-gray-500 absolute -top-5 bg-white/90 z-20 px-0.5 rounded whitespace-nowrap">
                      {val > 0 ? (Number.isInteger(val) ? val : val.toFixed(1)) : ''}
                    </div>
                    <div className="w-full h-full bg-gray-50 rounded-t-md relative overflow-hidden">
                      <div 
            ...
            {/* X-Axis Labels */}
            <div className={`w-full flex justify-between ${range === 'monthly' ? 'gap-2' : 'gap-4'} mt-2`}>
              {data.map((day, i) => (
                <div key={i} className="text-[10px] font-bold text-gray-400 truncate text-center flex-1 min-w-[20px]">
                  {day.label}
                </div>
              ))}
            </div>
